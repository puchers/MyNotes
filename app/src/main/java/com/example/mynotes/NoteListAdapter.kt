    ...
    inner class NoteViewHolder(private var binding: NoteListItemBinding) :
        ...
        fun bind(note: Note) {
            ...
            if (searchQuery.value.toString().isNotBlank()) {
                val highlightedText = note.noteTitle.replace(
                    searchQuery.value.toString(),
                    "<span style=\"background:yellow\">${searchQuery.value.toString()}</span>",
                    true
                )
            ...
            if (searchQuery.value.toString().isNotBlank()) {
                val highlightedText = note.noteBody.replace(
                    searchQuery.value.toString(),
                    "<span style=\"background:yellow\">${searchQuery.value.toString()}</span>",
                    true
                )
                binding.noteBody.text =
                    HtmlCompat.fromHtml(highlightedText, HtmlCompat.FROM_HTML_MODE_LEGACY)
            } else {
